...
Simulating an AR(_p_) model is also straightforward. One option is to use a `for()` loop like we did above for the [random walk](#simulating-random-walks) and [biased random walk](#simulating-a-biased-random-walk) models. Here we'll explore how to use the `arima.sim()` function in base R.
