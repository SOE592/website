...
**Note**: the distributional form for $\{w_t\}$ is flexible
...
where the following apply as well
...
```{r ex_rw, fig.cap="$x_t = x_{t-1} + w_t~$ with $~w_t \\sim \\text{N}(0,1)$"}
par(mfrow = c(1,2), mai = c(1.5,0.9,0.1,0.1), omi = c(0, 0, 0, 0))
tt <- cumsum(rnorm(100))
...
```{r ex_biased_rw, fig.cap="$x_t = x_{t-1} + 1 + w_t~$ with $~w_t \\sim \\text{N}(0,4)$"}
par(mfrow = c(1,2), mai = c(1.5,0.9,0.1,0.1), omi = c(0, 0, 0, 0))
xx <- ww <- rnorm(100, 0, 4)
uu <- 1
for(t in 2:100) {
...
## Aside: Differencing a time series

Differencing can be thought of like a derivative for discrete time series

Differencing is simply subtracting a time-shifted version of a time series from itself


## Notation: Differencing a time series

We use the _difference operator_ $\nabla$ to indicate a differencing operation
...
```{r ex_diff_biased_rw, fig.cap="$x_t - x_{t-1} = 1 + w_t~$ with $~w_t \\sim \\text{N}(0,1)$"}
par(mfrow = c(1,2), mai = c(1.5,0.9,0.1,0.1), omi = c(0, 0, 0, 0))
...
We saw previously that linear filters are a useful way of modeling time series
...
par(mfrow = c(2, 2),
    mai = c(0.7, 0.7, 0.3, 0.1), omi = c(0, 0, 0, 0))
...
To be stationary, __all roots__ of the characteristic equation __must exceed 1__
