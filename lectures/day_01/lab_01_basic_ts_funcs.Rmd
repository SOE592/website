...
Let's use `diff()` to remove the trend and seasonal signal from the CO<sub>2</sub> time series, beginning with the trend.  Close inspection of Figure \@ref(fig:plotdata1) would suggest that there is a nonlinear increase in CO<sub>2</sub> concentration over time, so we'll set `differences = 2`):
...
```{r plotCO2diff2eval, eval = TRUE, echo = FALSE}
## twice-difference the CO2 data
co2_d2 <- diff(co2, differences = 2)
```
  

(ref:plotCO2diff2) Time series of the twice-differenced atmospheric CO<sub>2</sub> concentration at Mauna Loa, Hawai'i.

```{r plotCO2diff2, eval = TRUE, echo = FALSE, fig.cap = '(ref:plotCO2diff2)'}
## set the margins & text size
...
We were apparently successful in removing the trend, but the seasonal effect still appears obvious (Figure \@ref(fig:plotCO2diff2)).  Therefore, let's go ahead and difference that series at lag-12 because our data were collected monthly.
...
```{r plotCO2diff12eval, eval = TRUE, echo = FALSE}
## difference the differenced CO2 data
co2_d2d12 <- diff(co2_d2, lag = 12)
```
  

(ref:plotCO2diff12) Time series of the lag-12 difference of the twice-differenced atmospheric CO<sub>2</sub> concentration at Mauna Loa, Hawai'i.

```{r plotCO2diff12, eval = TRUE, echo = FALSE, fig.cap = '(ref:plotCO2diff12)'}
## set the margins & text size
par(mar = c(4, 4.5, 1, 1), oma = c(0, 0, 0, 0), cex = 1)
...
Now we have a time series that appears to be random errors without any obvious trend or seasonal components (Figure \@ref(fig:plotCO2diff12)).
